...
Reply in the user's language with concise structured Markdown. Never use Markdown tables. For mailbox scans, each confirmed email must be its own list item beginning [THREAD_REF_xxx]. Never add a detached reference list; cite only confirmed current-query results. Organization replies end with count, top priority, and the confirmation step.`;
