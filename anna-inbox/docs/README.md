# Anna Inbox 文档索引

本目录只保留当前实现文档和明确标注的未来路线图。代码和 manifest 是最终事实来源。

当前基线：**App `2.1.7` / Tool `2.2.7`**（版本解耦）。

## 当前实现

- [2.2.7 架构与发布基线](2.2.7架构与发布基线.md)
- [邮箱同步 P0：缓存优先](邮箱同步P0缓存优先方案.md)
- [邮件内容预处理与完整线程回复](邮件内容预处理与完整线程回复.md)
- [AI 侧栏统一与 Brief 下线](AI侧栏统一与Brief下线.md)
- [AI 侧栏 Sampling → Sessions](AI侧栏Sampling转Sessions改造方案.md)（已实现）
- [AI 侧栏本地测试开关](AI侧栏本地测试开关.md)
- [AI 对话本地 Router 与白名单工具](AI对话本地Router与白名单工具设计.md)（详情/兼容路径；侧栏主路径为 Host / local Agent）
- [多邮箱接入](多邮箱接入方案.md)
- [邮件刷新与 All-mail 缓存](邮件刷新与All-mail缓存策略.md)
- [邮件详情抽屉](邮件详情抽屉改造方案.md)
- [富文本编辑工具栏](富文本编辑工具栏设计方案.md)
- [附件下载与外部链接](附件下载与外部链接打开方案.md)
- [附件下载与外发上传](附件下载与外发上传.md)
- [浏览器存储分层与正文预热](浏览器存储分层与正文预热方案.md)
- [平台运行性能诊断与优化进度](平台运行性能诊断与优化进度.md)
- [平台超时诊断与反馈流程](平台超时诊断与反馈流程.md)
- [联系人记忆](联系人记忆架构设计.md)
- [PyInstaller 打包](PyInstaller二进制打包指南.md)

## 调研与演进（未承诺上线）

- [AI 生产结构优化方案](AI生产结构优化方案.md)
- [AI 生产结构多方案选型调研](AI生产结构多方案选型调研.md)

## 路线图

`roadmap/` 中的内容没有发布承诺，也不能当作现有功能：

- [Draft 质量](roadmap/Draft质量优化方案.md)
- [Outlook 接入](roadmap/Outlook邮箱接入方案.md)

历史 Brief 管线、旧意图路由、旧 Ask 重构单页与过期版本基线已清理；仍有效约束已并入当前基线与 P0 同步文档。
