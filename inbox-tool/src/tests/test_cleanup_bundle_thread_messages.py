...
def main() -> None:
    ...
    check("full bundled keeps unread messages", len(full_bundled) == 3, f"got {len(full_bundled)}")
    check("card total count", card is not None and card.bundled_count == 3, f"got {getattr(card, 'bundled_count', None)}")
    check("card preview remains compact", card is not None and len(card.bundled_messages) == 3)
    check("only unread message ids are represented", {item["message_id"] for item in full_bundled} == {f"msg_{i}" for i in range(3)})
